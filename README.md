...
One page, two nodes. Each is a titled part of the sheet with its sections numbered inside it, laid on one four-column grid so edges line up from the room's readings down to the phone's track: tiles take a column, charts and tables two, a history all four. The window — the one control that changes what every number means — sits in a bar that stays at the top while the page is read, beside the pulse of each node. Open alerts sit under it, one line each.

- **Room** — now (temperature as the one hero figure on the sheet, humidity, the derived quantities, the station glyph); history; the window (distribution as a box with whiskers and the live value on the same axis, record integrity, provenance); record health (gaps found from the readings, boots).
- **Phone** — environment (station pressure and its three-hour tendency in the Met Office's words, pressure reduced to sea level with a Zambretti outlook labelled as the 1915 rule of thumb it is, light, noise as LAeq, L10 and L90, height change from the barometer); motion (acceleration, rotation, heading, and vibration events counted, listed and polled every ten seconds); position and coverage (altitude from the latest fix with its age, the serving cell and the broker's round trip, the share of the window in each signal band, and the track, drawn relative to its own start with no base map so a screenshot shows a shape rather than an address — fixes rougher than ±25 m left out, distance counted only for moves larger than the fixes are sure of).

**One axis per plot.** Two quantities in different units share a time axis as stacked plots, never one plot with a scale on each side, where the lines would cross wherever the two scales happened to line up and read as a relationship. The cursor moves through a stack together, and the value under it — or the latest, when nothing is hovered — is written above each plot, numbers first.
